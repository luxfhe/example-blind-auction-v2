...
task("task:bid")
  ...
  .setAction(async function (taskArguments: TaskArguments, hre) {
    ...
    const fherc20 = await deployments.get("ExampleToken");
    ...
    const encryptedAmount = await luxfhe.encrypt_uint32(amountToBid);

    let contractWithSigner = tokenContract.connect(
      signer,
    ) as unknown as FHERC20;

    console.log(
      `setting allowance on token contract: ${fherc20.address} for auction contract at: ${fherc20.address}`,
    );

    console.time("allowanceDuration");
    try {
      await contractWithSigner.approveEncrypted(
        auction.address,
        encryptedAmount,
      );
    } catch (e) {
    ...
    console.log(
      `bidding @ auction contract at: ${fherc20.address}, amount: ${amountToBid}`,
    );
