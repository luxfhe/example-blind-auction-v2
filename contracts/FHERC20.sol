...
contract FHERC20 is IFHERC20, ERC20, Permissioned {
    ...
    euint32 private totalEncryptedSupply;

    constructor(
        string memory name,
        string memory symbol
    ) ERC20(name, symbol) {
        totalEncryptedSupply = FHE.asEuint32(0);
    }
    ...
    /// @notice Request unwrap - first call to initiate async decrypt
    function requestUnwrap(uint32 amount) public {
        ...
        FHE.decrypt(amountToUnwrap);
    }

    /// @notice Complete unwrap after decrypt is ready
    function completeUnwrap(uint32 amount) public {
        euint32 encAmount = FHE.asEuint32(amount);
        euint32 amountToUnwrap = FHE.select(FHE.gt(_encBalances[msg.sender], encAmount), FHE.asEuint32(0), encAmount);

        // Get the decrypted result and mint
        uint32 decryptedAmount = FHE.reveal(amountToUnwrap);
        _mint(msg.sender, decryptedAmount);
    }

    /// @notice Unwrap in single call (for environments with sync decrypt)
    function unwrap(uint32 amount) public {
        ...
        // Request and get decrypt result
        FHE.decrypt(amountToUnwrap);
        uint32 decryptedAmount = FHE.reveal(amountToUnwrap);
        _mint(msg.sender, decryptedAmount);
    }
...
//    }
    ...
    function balanceOfEncrypted(
        ...
        return FHE.sealoutput(_encBalances[account], auth.publicKey);
    }
